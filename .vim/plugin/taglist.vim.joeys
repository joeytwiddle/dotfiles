...
" Space out the tree displayed in the taglist window
if !exists('Tlist_Well_Spaced')
    let Tlist_Well_Spaced = 0
endif

" Fold each tag in the tree
if !exists('Tlist_Fold_Tags')
    let Tlist_Fold_Tags = 1
endif

" Fold each tagtype in the tree
if !exists('Tlist_Fold_Tagtypes')
...
" java language
let s:tlist_java_ctags_args = '--language-force=java --java-types=picfm'
let s:tlist_java_tag_types = 'package interface class field method'
...
" Adds "s" or "es" on the end of a string.  Rules might need improving.
function! String_Pluralise(attype)
...
function! s:List_New(listname)
    if exists(a:listname."_count")
        echo "Warning: list" a:listname " already exists."
    else
...
" (Currently used to highlight tagtypes.)
function! Tlist_Highlight_Line(synType,lineNo)
    if has('syntax')
...
" Show every sub-tag that was found in the scope for a particular tag.
function! Tlist_Show_Scope_Contents(ftype,indent,ttype,tname,tnameshow)
    let start_fold = line('.') + 1

    " If it has no subtags then just print it and exit
    if exists("g:tlist_".a:ttype."_".a:tname."_scope_total")
        " echo "Got an existing one: ".a:ttype." ".a:tname
        if g:tlist_{a:ttype}_{a:tname}_scope_total == 0
            " echo "Got an empty one: ".a:ttype." ".a:tname
    ...
    " For each tag type:
    let ttso = s:tlist_{a:ftype}_tag_types . ' '
    while ttso != ''
        let attype = strpart(ttso, 0, stridx(ttso, ' '))
        let ttso = strpart(ttso, stridx(ttso, ' ') + 1)

        " Is there anything in the list of sub-tags for the given tag type?
        if exists("g:tlist_" . a:ttype . "_" . a:tname . "_scope_" . attype. "_count")
            let l:num = g:tlist_{a:ttype}_{a:tname}_scope_{attype}_count 
            if l:num > 0
                " echo a:indent . "For " . a:tname . " showing scope " . l:num
                if g:Tlist_Well_Spaced == 1
                    let print = nindent . "|"
                    silent! put =print
                endif
                let start_inner_fold = line('.') + 1
                let print = nindent . "|- " . String_Pluralise(attype) . ":"
                silent! put =print
                call Tlist_Highlight_Line("TagListTitle",line('.'))
                if ttso == ""
                ...
                if g:Tlist_Well_Spaced == 1
                    let print = String_Trim(nnindent."|")
                    silent! put =print
                endif

                " For every subtag of that tagtype in the scope:
                let j = 1
                while j <= g:tlist_{a:ttype}_{a:tname}_scope_{attype}_count
                    let subtagname = g:tlist_{a:ttype}_{a:tname}_scope_{attype}_{j}
...
function! s:Tlist_Explore_File(bufnum)
    ...
    if valid_cache != ''
        ...
        "" For debugging:
        let ctags_cmd = ctags_cmd . ' | tee /tmp/ctags.out'
        ...
        while cmd_output != ''
            ...
            " Extract the tag scope, if it is available. Tag scope is the last
            ...
            let tshow = tname
            " Extract the tag's prototype display if required:
            if g:Tlist_Display_Prototype == 1
                ...
                let tshow = String_Trim(strpart(one_line, start, end - start))
            endif
            ...
            call s:List_Add( "g:tlist_" . tscope_type . "_" . tscope . "_scope_" . ttype,tname)
            call s:List_Add( "g:tlist_" . tscope_type . "_" . tscope . "_scope_" . ttype . "_show",tshow)
            call s:List_Add( "g:tlist_" . tscope_type . "_" . tscope . "_scope_" . ttype . "_taglines",b:tlist_tag_count+1)
    ...
    normal "zE"
    call Tlist_Show_Scope_Contents(ftype,"","","","")
    " let i = 1
    " while i <= s:tlist_{ftype}_count
        " let ttype = s:tlist_{ftype}_{i}_name
        " " echo "Doing type: " . ttype
        " call Tlist_Show_Scope_Contents(ftype,"",ttype,"","")
        " let i = i + 1
    " endwhile
